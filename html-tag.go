...
func (f FileInput) BuildContainerView(id, field_name string, allow_skip_completed bool) string {
	...
	tags += `<button type="button" name="joystick" class="` + gamepad + `" title="joystick desconectado"` + disabled + `><svg aria-hidden="true" focusable="false" class="form-btn"><use xlink:href="#icon-joystick"/></svg></button>`
